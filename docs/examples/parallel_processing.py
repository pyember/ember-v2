...
async def main():
    ...
    summaries = await ember.parallel([summarize_document(doc) for doc in documents])
    ...
    for i, batch in enumerate(ember.batch(large_dataset, size=batch_size)):
        print(
            f"Processing batch {i+1}/{len(large_dataset)//batch_size + 1}...",
            end="",
            flush=True,
        )
    ...
    async def analyze_sentiment(text: str) -> SentimentAnalysis:
        ...
        return await ember.llm(
            f"Analyze the sentiment of: {text}", output_type=SentimentAnalysis
        )
    ...
    async def process_document_pipeline(doc: str) -> Dict[str, Any]:
        ...
        entities, sentiment = await ember.parallel(
            [extract_entities(doc), analyze_sentiment(doc)]
        )
    ...
    results = await ember.parallel([safe_process(text) for text in mixed_inputs])
