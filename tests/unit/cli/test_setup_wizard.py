...
class TestSetupWizardConfig:
    ...
    def test_provider_configuration_structure(self):
        ...
        setup_wizard_dir = Path(__file__).parent.parent.parent.parent / "src" / "ember" / "cli" / "setup-wizard"
        ...
        for field in required_fields:
            ...
            assert f"{field}:" in content, \
                f"Provider type should have {field} field"
        
        # Verify providers are defined
        for provider in ["openai", "anthropic", "google"]:
            assert f'"{provider}"' in content or f"'{provider}'" in content, \
                f"Provider {provider} should be defined"
    
    def test_setup_mode_options(self):
        """Test that setup modes are properly defined."""
        setup_wizard_dir = Path(__file__).parent.parent.parent.parent / "src" / "ember" / "cli" / "setup-wizard"
        setup_mode_file = setup_wizard_dir / "src" / "components" / "steps" / "SetupModeSelection.tsx"
        ...
        assert "'single'" in content or '"single"' in content, \
        ...
        assert "onSelectMode: (mode: 'single' | 'all')" in content, \
