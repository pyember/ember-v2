...
class TestModelRegistry:
    ...
    def test_api_key_retrieval(self, mock_getenv):
        ...
        mock_getenv.side_effect = lambda x: "test-key" if x == "OPENAI_API_KEY" else None
        key = registry._get_api_key("openai")
        assert key == "test-key"
        
        # Test Ember-specific format
        mock_getenv.side_effect = lambda x: "ember-key" if x == "EMBER_ANTHROPIC_API_KEY" else None
    ...
    def test_usage_tracking(self):
        ...
        # Mock model
        mock_model = Mock()
        usage1 = UsageStats(prompt_tokens=10, completion_tokens=20, total_tokens=30, cost_usd=0.001)
        usage2 = UsageStats(prompt_tokens=15, completion_tokens=25, total_tokens=40, cost_usd=0.002)
