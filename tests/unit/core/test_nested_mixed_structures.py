...
class MixedEnsemble(Module):
    ...
    def __init__(self, name: str, num_static: int, num_dynamic: int, 
                 dim: int, key: jax.random.PRNGKey):
        self.name = name
        ...
        self.dynamic_ops = [
            LearningOperator(f"dynamic_{i}", dim, keys[i]) 
...
def test_no_static_warnings_complex():
    ...
    with warnings.catch_warnings(record=True) as w:
        ...
        assert len(static_warnings) == 0, f"Got {len(static_warnings)} static warnings during creation"
...
def test_deep_nesting_compilation():
    ...
    shallow_time = timeit.timeit(
        lambda: process_first(shallow, jnp.ones(4)),
        number=100
    ) / 100
    
    deep_time = timeit.timeit(
        lambda: process_first(deep, jnp.ones(4)), 
        number=100
    ) / 100
