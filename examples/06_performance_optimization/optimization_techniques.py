...
def example_jit_speedup():
    ...
    def analyze_sentiment_detailed(text: str) -> dict:
        ...
        sentiment = models("gpt-3.5-turbo", sentiment_prompt).text.strip()
        
        # Emotion detection
        emotion_prompt = f"Primary emotion (joy/anger/sadness/fear/surprise): {text}"
        emotion = models("gpt-3.5-turbo", emotion_prompt).text.strip()
        
        # Intensity
        intensity_prompt = f"Sentiment intensity (1-10): {text}"
        intensity_text = models("gpt-3.5-turbo", intensity_prompt).text
...
def example_batch_processing():
    ...
    def classify_topic(text: str) -> str:
        ...
        return models("gpt-3.5-turbo", prompt).text.strip()
...
def example_caching_pattern():
    ...
    def cached_translate(text: str, target_lang: str = "Spanish") -> str:
        ...
        result = models("gpt-3.5-turbo", prompt).text.strip()
...
def example_optimized_pipeline():
    ...
    def process_document(doc: str) -> dict:
        ...
        # Summarize
        summary = models("gpt-3.5-turbo", f"Summarize in one sentence: {doc}").text
        
        # Extract key points
        points = models("gpt-3.5-turbo", f"List 3 key points: {doc}").text
        
        # Determine category
        category = models("gpt-3.5-turbo", f"Category (news/blog/research): {doc}").text
...
def main():
    ...
    if not any(os.environ.get(key) for key in ['OPENAI_API_KEY', 'ANTHROPIC_API_KEY', 'GOOGLE_API_KEY']):
        ...
        print("Performance optimization patterns shown:")
        print("• JIT compilation for repeated operations")
