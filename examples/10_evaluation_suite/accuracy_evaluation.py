...
def main():
        ...
        def forward(self, inputs):
            ...
            if task_type == "classification":
                ...
                test_cases = [
                    ...
                    {
                        "text": "Mixed feelings - great features but poor build.",
                        "label": "neutral",
                        "difficulty": "hard",
                    },
                    {
                    ...
                    {
                        "text": "Decent value for the price point.",
                        "label": "neutral",
                        "difficulty": "medium",
                    },
                    {
                        "text": "Revolutionary product that changes everything!",
                        "label": "positive",
                        "difficulty": "easy",
                    },
                    {
                        "text": "Complete disaster, nothing works.",
                        "label": "negative",
                        "difficulty": "easy",
                    },
                    {
                        "text": "Has pros and cons, depends on use case.",
                        "label": "neutral",
                        "difficulty": "hard",
                    },
                ]
    ...
    class SimpleSentimentClassifier(operators.Operator):
        ...
        def forward(self, inputs):
            ...
            confidence = max(pos_score, neg_score, neutral_score, 1) / (
                pos_score + neg_score + neutral_score + 1
            )
    ...
    class EvaluationPipeline(operators.Operator):
        ...
        system: operators.Operator
        evaluator: operators.Operator

        def __init__(
            self, *, system: operators.Operator, evaluator: operators.Operator
        ):
            self.system = system
    ...
    eval_pipeline_classification = EvaluationPipeline(
        system=classifier, evaluator=AccuracyEvaluator()
    )

    eval_pipeline_math = EvaluationPipeline(
        system=math_solver, evaluator=AccuracyEvaluator()
    )

    print("Running Classification Evaluation:")
    classification_results = eval_pipeline_classification(
        {"dataset": classification_data["test_cases"], "task_type": "classification"}
    )
    ...
    print(
        f"Correct: {classification_results['overall_metrics']['correct']}/{classification_results['overall_metrics']['total']}"
    )
    ...
    if "class_metrics" in classification_results["overall_metrics"]:
        print("\nPer-class metrics:")
        for cls, metrics in classification_results["overall_metrics"][
            "class_metrics"
        ].items():
            print(
                f"  {cls}: F1={metrics['f1']:.2f}, Precision={metrics['precision']:.2f}, Recall={metrics['recall']:.2f}"
            )

    print("\n" + "-" * 30 + "\n")
    print("Running Math Evaluation:")
    math_results = eval_pipeline_math(
        {"dataset": math_data["test_cases"], "task_type": "math"}
    )

    print(f"\nOverall Accuracy: {math_results['overall_metrics']['accuracy']:.2%}")
    print(
        f"Correct: {math_results['overall_metrics']['correct']}/{math_results['overall_metrics']['total']}"
    )
    print("\nPer-difficulty accuracy:")
    ...
    class ComparisonEvaluator(operators.Operator):
        ...
        systems: Dict[str, operators.Operator]
        pipeline: Dict[str, EvaluationPipeline]
        ...
        def __init__(self, *, systems: Dict[str, operators.Operator]):
            ...
            for name, system in systems.items():
                self.pipeline[name] = EvaluationPipeline(
                    system=system, evaluator=AccuracyEvaluator()
                )
    ...
    comparator = ComparisonEvaluator(
        systems={"rule_based": classifier, "random_baseline": RandomClassifier()}
    )
    ...
    print(
        f"Statistically significant: {'Yes' if comparison['is_significant'] else 'No'}"
    )
