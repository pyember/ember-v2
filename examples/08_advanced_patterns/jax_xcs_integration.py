...
def main():
    ...
    class MixedOperator(operators.Operator):
        ...
        weights: jax.Array  # Dynamic (JAX arrays are automatically learnable)
        bias: jax.Array  # Dynamic
        config: dict  # Static (non-JAX types are automatically static)
        threshold: float  # Static
        ...
        def forward(self, x: jnp.ndarray) -> Dict:
            ...
            return {"score": float(score), "action": action, "model": self.model_name}
    ...
    def loss_fn(
        weights: jnp.ndarray, bias: jnp.ndarray, x: jnp.ndarray, target: float
    ) -> float:
    ...
    class LearnedPromptRouter(operators.Operator):
        ...
        # Field declarations
        routes: Dict[str, str]  # Static
        models: Dict[str, str]  # Static
        route_embeddings: jax.Array  # Dynamic (learnable)
        projection: jax.Array  # Dynamic (learnable)
        temperature: jax.Array  # Dynamic (learnable)
    ...
    def routing_loss(params, prompt, target_route_idx):
        """Loss function for training the router."""
        # Create a new router with updated parameters
        route_embeddings, projection = params
        updated_router = router.update_params(
            route_embeddings=route_embeddings, projection=projection
        )

        prompt_embedding = updated_router.encode_prompt(prompt)
        scores = updated_router.compute_routing_scores(prompt_embedding)

        # Cross-entropy loss
        return -jnp.log(scores[target_route_idx] + 1e-7)

    # Compute gradients for router parameters
    grad_fn = jax.grad(routing_loss, argnums=0)
    ...
    params = (router.route_embeddings, router.projection)
    ...
    # Show complete optimization loop with optax
    print("\nComplete optimization example with optax:")
    print("```python")
    print("import optax")
    print("")
    print("# Initialize optimizer")
    print("optimizer = optax.adam(learning_rate=0.01)")
    print("opt_state = optimizer.init(router)")
    print("")
    print("# Training loop")
    print("for epoch in range(100):")
    print("    # Compute loss and gradients")
    print("    loss_val = routing_loss(params, prompt, target_idx)")
    print("    grads = grad_fn(params, prompt, target_idx)")
    print("    ")
    print("    # Update parameters with optax")
    print("    updates, opt_state = optimizer.update(grads, opt_state)")
    print("    ")
    print("    # Apply updates to router using update_params")
    print("    router = router.update_params(")
    print("        route_embeddings=router.route_embeddings + updates[0],")
    print("        projection=router.projection + updates[1]")
    print("    )")
    print("```")

    # Actually run a simple optimization example
    try:
        import optax
        ...
        for i in range(3):
            # Get current params
            current_params = (
                current_router.route_embeddings,
                current_router.projection,
            )

            # Compute loss and gradients
            loss_val = routing_loss(current_params, prompt, target_idx)
            grads = grad_fn(current_params, prompt, target_idx)

            # Update with optax
            updates, opt_state = optimizer.update(grads, opt_state)

            # Apply updates
            current_router = current_router.update_params(
                route_embeddings=current_params[0] + updates[0],
                projection=current_params[1] + updates[1],
            )

            print(f"  Step {i}: loss = {loss_val:.4f}")

    except ImportError:
        print("\n💡 Note: Install optax to run optimization: pip install optax")
