...
def example_jit_with_complex_function():
    ...
    def analyze_and_summarize(article: str, max_points: int = 3) -> dict:
        ...
        # Determine category
        category_prompt = (
            f"Categorize this article (tech/business/science/other): {article}"
        )
        category = models("gpt-3.5-turbo", category_prompt).text

        return {"key_points": points, "summary": summary, "category": category}
...
def example_conditional_optimization():
    ...
    def smart_responder(query: str, verbose: bool = False) -> str:
        ...
        classification = models(
            "gpt-3.5-turbo", f"Classify as question/statement/command: {query}"
        ).text
        ...
        if "question" in classification.lower():
            ...
            if verbose:
                explanation = models(
                    "gpt-3.5-turbo", f"Explain the answer: {response}"
                ).text
