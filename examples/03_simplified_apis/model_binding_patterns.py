...
def main():
    ...
    for q in questions[:2]:
        ...
        response = models("gpt-3.5-turbo", f"Explain briefly: {q}", temperature=0.7, max_tokens=50)
        print(f"Q: {q}")
        print(f"A: {response.text[:50]}..." if len(response.text) > 50 else f"A: {response.text}")
    ...
    creative_writer = models.instance("gpt-3.5-turbo", 
                                     temperature=0.9,
                                     system="You are a creative writer. Be imaginative and poetic.")
    
    technical_expert = models.instance("gpt-3.5-turbo",
                                      temperature=0.2,
                                      system="You are a technical expert. Be precise and accurate.")
    
    concise_assistant = models.instance("gpt-3.5-turbo",
                                       temperature=0.5,
                                       max_tokens=30,
    ...
    analyzer = models.instance("gpt-3.5-turbo", 
                              temperature=0.3,
                              system="You are a text analyzer. Identify key themes.")
    
    summarizer = models.instance("gpt-3.5-turbo",
                                temperature=0.5,
    ...
    def analyze_and_summarize(text: str) -> dict:
        ...
        summary = f"This text discusses {text[:20]}... in the context of modern developments."
