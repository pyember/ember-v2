...
from ember.api.types import EmberModel, Field, model_validator, field_validator

# Note: Specification is deprecated - just use EmberModel types directly!
...
def main(_simulated_mode=False):
    ...
    def search_documents(query: SearchQuery) -> List[SearchResult]:
        ...
        if _simulated_mode:
            ...
            response = models("gpt-4", f"Search for documents matching: {query.query}")
    ...
    # Modern pattern: Just use type annotations!
    @operators.op
    def analyze_investment(inputs: InvestmentAnalysisInput) -> InvestmentAnalysisOutput:
        """Complex analysis with rich types - no Specification class needed!"""
        if _simulated_mode:
            # Simulate analysis
            ...
            # Real API call with structured output
            prompt = f"""Analyze this investment portfolio:
            Portfolio: {inputs.portfolio}
            Market conditions: {inputs.market_conditions}
            Risk tolerance: {inputs.risk_tolerance}
            Analysis depth: {inputs.analysis_depth}
            
            Provide a structured analysis with summary, risk score, recommendations, and projections.
            """
            response = models(
                "gpt-4", prompt, structured_output=InvestmentAnalysisOutput
            )
            return response
    ...
    result = analyze_investment(analysis_input)
