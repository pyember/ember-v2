...
def main():
        ...
        def forward(self, inputs):
            ...
            for chunk in chunks:
                ...
                features = {
                    "length": len(words),
                    "ml_terms": sum(
                        1 for w in words if w in ["machine", "learning", "ml", "ai"]
                    ),
                    "deep_terms": sum(
                        1 for w in words if w in ["deep", "neural", "network"]
                    ),
                    "data_terms": sum(
                        1 for w in words if w in ["data", "dataset", "pattern"]
                    ),
                    "algo_terms": sum(
                        1 for w in words if w in ["algorithm", "model", "train"]
                    ),
                }
    ...
    class SemanticRetriever(operators.Operator):
        ...
        def forward(self, inputs):
            ...
            # Embed the query
            query_embedded = self.embedder(
                {"chunks": [{"text": query, "chunk_id": "query"}]}
            )
            ...
            for emb in embeddings:
                ...
                similarities.append({"chunk": emb["chunk"], "similarity": similarity})
    ...
    response = generator(
        {"query": query, "retrieved_chunks": retrieved["retrieved_chunks"]}
    )
    ...
    class RAGPipeline(operators.Operator):
        ...
        def __init__(
            self, *, documents: List[Dict], chunk_size: int = 50, top_k: int = 3
        ):
        ...
        def forward(self, inputs):
            ...
            retrieved = self.retriever(
                {"query": query, "embeddings": self.index["embeddings"]}
            )
            ...
            return {
                ...
                "sources": [
                    {"title": chunk["chunk"]["title"], "relevance": chunk["similarity"]}
