...
from ember.xcs._internal.ir import IRGraph, IRNode, ParallelismInfo, analyze_node_parallelism
...
class ParallelismAnalyzer:
    ...
    def _analyze_node(self, node: IRNode, graph: IRGraph) -> ParallelismInfo:
        ...
        if graph.has_independent_branches(node.id):
            ...
            info_dict['estimated_speedup'] = max(
                info_dict['estimated_speedup'],
                len(graph.get_dependents(node.id))
            )
    ...
    def _find_vectorizable_chains(self, graph: IRGraph, 
                                  node_info: Dict[str, ParallelismInfo]) -> List[List[str]]:
                ...
                while True:
                    ...
                    if len(dependents) == 1:
                        ...
                        if (next_id not in visited and 
                            node_info[next_id].can_vmap and
                            self._are_compatible_for_fusion(graph, current, next_id)):
    ...
    def _find_bottlenecks(self, graph: IRGraph, 
                          node_info: Dict[str, ParallelismInfo],
                          parallel_groups: List[Set[str]]) -> List[str]:
    ...
    def _estimate_speedup(self, node_info: Dict[str, ParallelismInfo],
                          parallel_groups: List[Set[str]],
                          vectorizable_chains: List[List[str]]) -> float:
