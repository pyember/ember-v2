...
from ember.xcs._internal.tracer import Operation, PythonTracer, TracingError
...
class IRBuilder:
    ...
    def _build_graph_from_operations(self, operations: List[Operation]) -> IRGraph:
        ...
        for op in operations:
            ...
            # Determine if this is an orchestration operation first
            is_orchestration = False

            # Check if it's an Ember API function (models, operators, etc)
            if hasattr(op.func, "__module__"):
                module = str(op.func.__module__)
                if "ember" in module or "langchain" in module or "openai" in module:
                    is_orchestration = True

            # Check function name patterns
            func_name = getattr(op.func, "__name__", "")
            if any(pattern in func_name.lower() for pattern in ["model", "llm", "api", "request"]):
                is_orchestration = True
            ...
            # If no dependencies, this operation needs literal values
            # For orchestration ops, we'll handle this at execution time
            # For pure functions, we can use the stored args
            if not inputs and not is_orchestration:
                # Create placeholders for the arguments
                for i in range(len(op.args)):
                    inputs.append(f"_literal_{node_id}_{i}")

            # Create output variable
            output_var = f"{node_id}_out"

            # Create IR node metadata
            metadata = {
                "op_type": "traced",
                "arg_count": len(op.args),
                "has_kwargs": bool(op.kwargs),
            }

            # For orchestration operations, we need special handling
            if is_orchestration:
                # Store the original call info
                metadata["is_orchestration"] = True
                # Store the actual args for orchestration operations
                # These will be used at execution time
                metadata["orchestration_args"] = op.args
                metadata["orchestration_kwargs"] = op.kwargs
            else:
                # Pure function - safe to store args for replay
                metadata["args"] = op.args
                metadata["kwargs"] = op.kwargs
    ...
    def add_operation(self, operator: Any, inputs: List[str]) -> List[str]:
        ...
        # Create node
        node = IRNode(id=node_id, operator=operator, inputs=tuple(inputs), outputs=tuple(outputs))
    ...
    def build(self) -> IRGraph:
        ...
        return IRGraph(nodes=dict(self.state.nodes), edges=frozen_edges)
