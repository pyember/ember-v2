...
def example_functions_as_operators():
    ...
    def classify_email(email: str) -> dict:
        ...
        # Determine urgency
        urgency = models("gpt-3.5-turbo", f"Rate urgency (high/medium/low): {email}").text
        
        # Detect intent  
        intent = models("gpt-3.5-turbo", f"What's the intent (question/request/info): {email}").text
...
def main():
    ...
    if not any(os.environ.get(key) for key in ['OPENAI_API_KEY', 'ANTHROPIC_API_KEY', 'GOOGLE_API_KEY']):
        ...
        print("Showing the natural API principles instead...\n")
        
        print("1. Functions are operators - no base classes")
        print("2. Compose with normal Python - no special syntax")
        print("3. Optimize with @jit - no configuration")
        print("4. Batch with vmap() - no special APIs")
        print("5. Use models naturally - models('gpt-4', prompt)")
        return
