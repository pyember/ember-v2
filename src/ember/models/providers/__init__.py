...
def get_provider_class(name: str) -> Type[BaseProvider]:
    ...
    raise ValueError(
        f"Unknown provider '{name}'. " f"Available providers: {', '.join(sorted(available))}"
    )
...
def resolve_model_id(model: str) -> tuple[str, str]:
    ...
    if (
        ...
        or model_lower.startswith("o1-")  # New reasoning models
        or model_lower in ["gpt-4o", "gpt-4o-mini"]
    ):
        return "openai", model
