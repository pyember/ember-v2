    ...
    def needs_adaptation(config: Dict[str, Any]) -> bool:
        ...
        if "providers" in config:
            for provider in config["providers"].values():
                if isinstance(provider, dict) and "envKey" in provider and "api_key" not in provider:
                    return True
        ...
        if "providers" in adapted:
            for name, provider in adapted["providers"].items():
                if isinstance(provider, dict):
                    adapted["providers"][name] = CompatibilityAdapter.adapt_provider_config(provider)
